...
public interface Facade <X extends Identifiable<K>, K, F, A >{
    ...
    X cria(@NotNull @Valid final X elemento)
        throws BusinessException;

    X pega(@NotNull @Valid final K key)
        throws EntidadeInexistenteException;

    @Transactional(readOnly = true)
    Page<X> lista(final F filtro, final Pageable pageable)
            throws EntidadeInexistenteException;

    @Transactional(readOnly = true)
    default Page<X> lista() throws EntidadeInexistenteException{
    ...
    @Transactional(rollbackFor={RuntimeException.class,BusinessException.class})
    void deleta(@NotNull @Valid final K key)
        throws BusinessException;
}
